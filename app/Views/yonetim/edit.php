...
<body class="flex flex-col h-screen overflow-hidden">
    ...
    <header class="h-16 border-b-2 border-[var(--text-main)] flex justify-between items-center px-6 md:px-12 bg-[var(--bg-paper)] z-50 flex-shrink-0 transition-colors">
        <div class="flex items-center gap-3">
            <a href="/yonetim/dashboard" class="font-mono text-[10px] uppercase border border-[var(--text-main)] px-3 py-1 hover:bg-[var(--text-main)] hover:text-[var(--bg-paper)] transition-colors text-[var(--text-main)]">← PANEL</a>
    ...
    <div id="writePanel" class="flex flex-1 overflow-hidden">
        ...
        <main class="flex-1 p-6 md:p-12 overflow-y-auto">
            <form id="editForm" action="/yonetim/update" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto space-y-8 pb-20">
                <?= Csrf::field() ?>
                ...
                <div>
                    ...
                    <div class="flex justify-end mb-2">
                        ...
                        <button type="button" onclick="document.getElementById('editWordInput').click()"
    ...
    <script>
            ...
            $.ajax({data,type:"POST",url:"/yonetim/upload-content-image",cache:false,contentType:false,processData:false,dataType:"json",
                success: function(res) {
                    if (!res || !res.success || !res.url) {
                        alert(res && res.error ? ('Hata: ' + res.error) : 'Yükleme başarısız.');
                        return;
                    }
                    const url = res.url;
                ...
                error: function(xhr) {
                    if (xhr.status === 403) {
                        alert('Oturum süresi dolmuş. Sayfayı yenileyin.');
                        return;
                    }
                    try {
                        const j = JSON.parse(xhr.responseText || '{}');
                        alert('Hata: ' + (j.error || ('Sunucu hatası ' + xhr.status)));
                    } catch (e) {
        ...
        window.FezadanAdminEditor && window.FezadanAdminEditor.init({
            ...
            coverInputSelector: '#coverFileInput',
            coverPreviewSelector: '#currentCoverThumb',
