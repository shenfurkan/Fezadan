...
<body class="h-screen flex flex-col relative overflow-hidden">
    ...
    <header class="h-20 border-b-2 border-[var(--text-main)] flex justify-between items-center px-6 md:px-12 sticky top-0 bg-[var(--bg-paper)] z-50 flex-shrink-0 transition-colors">
        <div class="flex items-center gap-3">
            <a href="/yonetim/dashboard" class="font-mono text-[10px] uppercase border border-[var(--text-main)] px-3 py-1 hover:bg-[var(--text-main)] hover:text-[var(--bg-paper)] transition-colors">&lt; PANEL</a>
            <a href="/yonetim/authors"   class="font-mono text-[10px] uppercase border border-[var(--text-accent)] text-[var(--text-accent)] px-3 py-1 hover:bg-[var(--text-accent)] hover:text-[var(--bg-paper)] transition-colors font-bold">YAZARLARI YÖNET</a>
    ...
    <form id="uploadForm" action="/yonetim/store" method="POST" enctype="multipart/form-data"
    ...
    <script>
        ...
        function uploadContentImage(file) {
            return new Promise((resolve,reject) => {
                ...
                $.ajax({data,type:"POST",url:"/yonetim/upload-content-image",cache:false,contentType:false,processData:false,dataType:"json",
                    success: function(res) {
                        if (res && res.success && res.url) {
                            resolve(res.url);
                        } else {
                            reject((res && res.error) ? res.error : "Yükleme başarısız.");
                        }
                    },
                    error: function(xhr) {
                        if (xhr.status === 403) {
                            reject("Oturum süresi dolmuş. Sayfayı yenileyip tekrar deneyin.");
                        } else {
                            try {
                                const j = JSON.parse(xhr.responseText || '{}');
                                reject(j.error || ("Sunucu hatası: " + xhr.status));
                            } catch (e) {
                                reject("Sunucuya ulaşılamadı. Hata kodu: " + xhr.status);
                            }
                        }
                    }
                });
        ...
        function appendToSummernote(html) {
            const current = $('#summernote').summernote('code');
            const isEmpty = !current || current === '<p><br></p>' || current.trim() === '';
            $('#summernote').summernote('code', isEmpty ? html : current + '<p><br></p>' + html);
        }
        ...
        document.getElementById('wordInput').addEventListener('change', function() {
            ...
            reader.onload=function(e){
                mammoth.convertToHtml({arrayBuffer:e.target.result}).then(async function(result){
                    ...
                    appendToSummernote(doc.body.innerHTML);
