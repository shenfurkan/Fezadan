...
<body class="flex h-screen w-full overflow-hidden relative">
    ...
    <div class="scroll-smooth h-full"> <?php if (isset($_GET['status'])): ?>
        <div class="mb-8 p-6 bg-[#6D2323] text-[#FEF9E1] border-2 border-[var(--text-main)] shadow-[8px_8px_0px_#A31D1D] flex items-center justify-between">
            ...
            <a href="/yonetim/dashboard" class="px-4 py-2 border border-[#FEF9E1] hover:bg-[var(--bg-paper)] hover:text-[var(--text-main)] font-mono text-xs uppercase transition-colors">TAMAM</a>
        </div>
        <?php endif; ?>
                            ...
                            <tr class="border-b border-[var(--text-main)]/10 hover:bg-[var(--bg-secondary)]/20 transition-colors h-20">
                                ...
                                <td class="p-4 text-center">
                                    <div class="flex justify-center gap-2 flex-col md:flex-row items-center">
                                        ...
                                        <?php if(($article['status'] ?? 'published') === 'draft'): ?>
                                        <form method="POST" action="/yonetim/publish?id=<?php echo $article['id']; ?>" onsubmit="return confirm('Makaleyi yayınlamak istiyor musun?');" class="contents">
                                            <?= Csrf::field() ?>
                                        ...
                                        <?php endif; ?>
                                        <form method="POST" action="/yonetim/delete?id=<?php echo $article['id']; ?>" onsubmit="return confirm('Silmek istediğine emin misin?');" class="contents">
                                            <?= Csrf::field() ?>
                        ...
                        <div class="mb-6 last:mb-0 relative pl-6 border-l-2 <?php echo $index === 0 ? 'border-[var(--text-accent)]' : 'border-[var(--text-main)]/30'; ?>">
                            ...
                            <div class="flex justify-between items-start mb-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form method="POST" action="/yonetim/patch-delete?id=<?php echo $patch['id']; ?>" onsubmit="return confirm('Bu yama notunu silmek istediğinize emin misiniz?');" class="inline">
                                        <?= Csrf::field() ?>
                                        <button type="submit" class="text-[10px] text-[var(--text-accent)] hover:text-black font-bold transition-colors">
                                            [SİL]
                                        </button>
                                    </form>
                                </div>
        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <a href="/yonetim/create-patch" class="group border-2 border-[var(--text-main)] hover:bg-[var(--text-accent)] hover:text-[#FEF9E1] hover:border-[var(--text-accent)] transition-all p-6 flex flex-col justify-between cursor-pointer h-32 relative overflow-hidden">
    ...
    const logs = [
        "Bağlantı kuruldu: <?php echo htmlspecialchars($_SERVER['REMOTE_ADDR'] ?? '', ENT_QUOTES, 'UTF-8'); ?>",
