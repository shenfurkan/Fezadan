...
<body class="flex h-screen w-full overflow-hidden relative">
    ...
    <?php if(isset($_GET['status'])): ?>
        ...
        <form action="/yonetim/update-password" method="POST" class="max-w-md space-y-8">
            <?= Csrf::field() ?>
            <div>
                <label class="block font-mono text-[10px] uppercase opacity-60">Aktif Kullanıcı</label>
                <div class="font-bold text-xl uppercase text-[var(--text-main)]"><?php echo htmlspecialchars($_SESSION['admin_user'] ?? 'yonetim'); ?></div>
            </div>
