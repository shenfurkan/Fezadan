...
<body class="flex h-screen w-full overflow-hidden relative">
    ...
    <div class="h-full flex flex-col">
        ...
        <?php if ($flashMessage = Flash::pull()): ?>
            <div class="mb-6 p-4 bg-[#6D2323] text-[#FEF9E1] font-mono text-xs flex-shrink-0 shadow-[4px_4px_0px_#A31D1D]">
        ...
        <?php if(isset($_GET['status']) && $_GET['status'] == 'deleted'): ?>
            <div class="mb-6 p-4 bg-[#6D2323] text-[#FEF9E1] font-mono text-xs uppercase flex-shrink-0 shadow-[4px_4px_0px_#A31D1D]">
                NOT VE PDF DOSYASI BASARIYLA SILINDI.
            </div>
        <?php endif; ?>
        <?php if(isset($_GET['message'])): ?>
            <?php if($_GET['message'] == 'duplicate_title'): ?>
                <div class="mb-6 p-4 bg-orange-600 text-[#FEF9E1] font-mono text-xs uppercase flex-shrink-0 animate-pulse shadow-[4px_4px_0px_#A31D1D]">
                    ⚠️ BU BAŞLIKTA BİR NOT ZATEN VAR. LÜTFEN FARKLI BİR BAŞLIK SEÇİN.
                </div>
            <?php elseif($_GET['message'] == 'limit_exceeded'): ?>
                <div class="mb-6 p-4 bg-red-600 text-[#FEF9E1] font-mono text-xs uppercase flex-shrink-0 shadow-[4px_4px_0px_#A31D1D]">
                            DOSYA BOYUTU LİMİTİ AŞILDI (MAKS. 50MB).
                </div>
            <?php endif; ?>
        <?php endif; ?>
        ...
        <div class="flex flex-col lg:flex-row gap-12 flex-1 min-h-0 h-[calc(100vh-180px)]">
            ...
            <div class="lg:w-2/5 flex flex-col h-full">
                ...
                <form action="/yonetim/store-note" method="POST" enctype="multipart/form-data" class="border-2 border-[var(--text-main)] bg-[var(--bg-paper)] p-6 shadow-[8px_8px_0px_#A31D1D] space-y-6 flex-1 overflow-y-auto custom-scrollbar">
                    <?= Csrf::field() ?>
                    ...
                    <div>
                        ...
                        <div id="file-limit-warning" class="hidden mt-2 p-2 bg-red-100 text-red-700 font-mono text-[10px] border-l-4 border-red-700 uppercase">
                            Dosya boyutu çok büyük (Maksimum 50MB). Lütfen daha küçük bir dosya seçin.
                        </div>
                    </div>

                    <div>
                        <label class="block font-syne font-bold uppercase text-xs mb-2">İçerik Detayları</label>
                        <textarea name="description" rows="5" class="brutalist-input" placeholder="Kısa bilgi girin..."></textarea>
                    </div>
                    ...
                    <?php if(!empty($notes)): foreach($notes as $note): ?>
                        <div class="bg-[var(--bg-paper)] border-2 border-[var(--text-main)] p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-[var(--bg-secondary)] transition-colors">
                            ...
                            <div class="flex gap-2 w-full md:w-auto">
                                <a href="/yonetim/edit-note?id=<?php echo $note['id']; ?>" class="flex-1 md:flex-none text-center px-3 py-2 bg-[var(--bg-secondary)] border-2 border-[var(--text-main)] text-[var(--text-main)] text-[10px] font-bold font-mono hover:bg-[var(--text-main)] hover:text-[#FEF9E1] transition-colors">
                                    DÜZENLE
                                </a>
                                ...
                                <form method="POST" action="/yonetim/delete-note" data-confirm="Bu notu ve PDF dosyasını kalıcı olarak silmek istediğinize emin misiniz?" class="flex-1 md:flex-none">
                                    <?= Csrf::field() ?>
                                    <input type="hidden" name="id" value="<?php echo (int)$note['id']; ?>">
                                    <button type="submit" class="w-full text-center px-3 py-2 bg-[var(--text-accent)] border-2 border-[var(--text-accent)] text-white text-[10px] font-bold font-mono hover:bg-black hover:border-black transition-colors">
                                        SİL
                                    </button>
                                </form>
                            </div>
...
<script nonce="<?= CSP_NONCE ?>">
    ...
    function addCategory(cat) {
        ...
        tag.innerHTML = `${cat.name} <span class="remove-cat-btn cursor-pointer hover:text-red-400" data-cat="${cat.id}">✕</span>`;
    ...
    tagsContainer.addEventListener('click', (e) => {
        ...
        if (!btn) return;
        const id = parseInt(btn.getAttribute('data-cat'));
        removeCategory(id, btn);
    });
