...
<body class="flex h-screen w-full overflow-hidden relative">
    ...
    <aside class="w-20 md:w-72 bg-[var(--bg-paper)] border-r-2 border-[var(--text-main)] flex flex-col z-20 flex-shrink-0 transition-all duration-300">
        <div class="h-24 flex items-center justify-center md:justify-start md:px-8 border-b-2 border-[var(--text-main)]">
            <div class="hidden md:block">
                ...
                <p class="font-mono text-[10px] text-[var(--text-accent)] tracking-[0.2em]">TERMINAL v3.0</p>
            </div>
            <div class="md:hidden font-syne text-2xl font-bold">FZD</div>
        </div>

        <nav class="flex-1 overflow-y-auto py-8 flex flex-col gap-2">
            <a href="/admin/dashboard" class="nav-item active py-4 px-8 font-bold uppercase tracking-widest text-sm flex items-center gap-3">
                <span class="text-lg">▣</span> <span class="hidden md:inline">Genel Bakış</span>
            </a>
            <a href="/admin/create" class="nav-item py-4 px-8 font-bold uppercase tracking-widest text-sm flex items-center gap-3">
                <span class="text-lg">✎</span> <span class="hidden md:inline">Yeni Makale Yaz</span>
            </a>
            <a href="/admin/profile" class="nav-item py-4 px-8 font-bold uppercase tracking-widest text-sm flex items-center gap-3">
                <span class="text-lg">⚙</span> <span class="hidden md:inline">Profil Ayarları</span>
            </a>
            <a href="/admin/categories" class="nav-item py-4 px-8 font-bold uppercase tracking-widest text-sm flex items-center gap-3">
                <span class="text-lg">❖</span> <span class="hidden md:inline">Kategoriler</span>
            </a>
            <a href="/admin/authors" class="nav-item py-4 px-8 font-bold uppercase tracking-widest text-sm flex items-center gap-3">
                <span class="text-lg">♟</span> <span class="hidden md:inline">Yazarlar</span>
            </a>
        </nav>

        <div class="p-6 border-t-2 border-[var(--text-main)]">
            <a href="/admin/logout" class="block border-2 border-[var(--text-accent)] text-[var(--text-accent)] text-center py-3 font-bold uppercase hover:bg-[var(--text-accent)] hover:text-[#FEF9E1] transition-colors text-xs tracking-widest">
                Güvenli Çıkış
            </a>
        </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col relative z-10 overflow-hidden">
        ...
        <header class="h-24 border-b-2 border-[var(--text-main)] bg-[var(--bg-paper)]/90 backdrop-blur-sm flex justify-between items-center px-6 md:px-12 flex-shrink-0">
            <div>
                <h2 class="font-syne text-xl md:text-3xl font-bold uppercase text-[var(--text-main)]">KONTROL PANELİ</h2>
                <div class="flex items-center gap-2 text-xs font-mono text-[var(--text-accent)] mt-1">
                    <span class="w-2 h-2 bg-[var(--text-accent)] rounded-full blink"></span>
                    SİSTEM ÇEVRİMİÇİ
                </div>
            </div>
            <div class="text-right hidden md:block">
                <p class="text-[10px] uppercase opacity-60 tracking-widest">YEREL ZAMAN</p>
                <p class="font-mono text-xl font-bold" id="clock">00:00:00</p>
            </div>
        </header>

        <div class="flex-1 overflow-y-auto p-6 md:p-12 scroll-smooth">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                <div class="brutalist-card bg-[var(--bg-paper)] p-6 relative group overflow-hidden flex flex-col justify-between">
                    ...
                    <div class="relative z-10 border-b border-[var(--text-main)]/10 pb-3 mb-3">
                        ...
                        <div class="font-syne text-4xl font-bold text-[var(--text-main)]">
            ...
            <div class="mb-12">
                ...
                <div class="border-2 border-[var(--text-main)] bg-white overflow-hidden shadow-[8px_8px_0px_rgba(109,35,35,0.1)]">
                    <table class="w-full text-left font-mono text-xs table-fixed">
                        <thead class="bg-[#6D2323] text-[#FEF9E1] uppercase">
                            <tr>
                                ...
                                <th class="bg-[#6D2323] w-[10%] border-r border-[#FEF9E1]/20 hidden md:table-cell cursor-pointer hover:bg-black transition-colors p-0">
                                ...
                                <th class="bg-[#6D2323] w-auto p-4 text-center">İşlem</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr class="border-b border-[var(--text-main)]/10 hover:bg-[var(--bg-secondary)]/20 transition-colors h-20">
                                    ...
                                    <td class="p-4 border-r border-[var(--text-main)]/10 font-bold font-sans text-sm italic group relative">
                                        <div class="truncate w-full absolute inset-0 p-4 flex items-center">
                                            <a href="/makale/<?php echo $article['slug']; ?>" target="_blank" class="hover:text-[var(--text-accent)] transition-colors truncate" title="<?php echo htmlspecialchars($article['title']); ?>">
                                    ...
                                    <td class="p-4 border-r border-[var(--text-main)]/10 hidden md:table-cell font-mono text-xs text-center">
                                        <div class="inline-flex items-center gap-2 bg-[var(--text-main)]/5 px-2 py-1 rounded border border-[var(--text-main)]/10">
                                            <span class="text-lg text-[var(--text-accent)]">👁</span> 
                                            <span class="font-bold"><?php echo number_format($article['reads']); ?></span>
                                        </div>
                                    </td>

                                    <td class="p-4 text-center">
                                        <div class="flex justify-center gap-2 flex-col md:flex-row items-center">
                                            <a href="/admin/edit?id=<?php echo $article['id']; ?>" class="w-full md:w-auto px-2 py-1 bg-[#6D2323] text-[#FEF9E1] hover:bg-[var(--bg-paper)] hover:text-[var(--text-main)] border border-[var(--text-main)] text-[10px] font-bold text-center">DÜZENLE</a>
                                            <a href="/admin/delete?id=<?php echo $article['id']; ?>" onclick="return confirm('Silmek istediğine emin misin?');" class="w-full md:w-auto px-2 py-1 bg-[var(--text-accent)] text-white hover:bg-black text-[10px] font-bold text-center">SİL</a>
                                        </div>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
            ...
            <footer class="mt-12 pt-6 border-t border-[var(--line-color)] text-center md:text-left flex justify-between items-center opacity-60">
