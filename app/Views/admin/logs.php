...
<body class="flex h-screen w-full overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto p-6 md:p-10">
        <?php
            ...
            $levelLabels = ['ERROR' => 'ERROR', 'WARNING' => 'WARNING', 'INFO' => 'INFO'];
        ?>
        <header class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
            <div>
                <p class="font-mono text-xs uppercase tracking-widest opacity-70"></p>
        ...
        <form method="GET" action="/yonetim/logs" class="log-card p-4 mb-6 grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            ...
            <label class="font-mono text-xs uppercase font-bold md:col-span-1">
                Ara
                <input class="log-input mt-2" name="q" value="<?php echo htmlspecialchars((string)($filters['q'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="request_id, file, import">
            </label>
        ...
        <?php if (empty($logs)): ?>
            <section class="log-card p-8">
                ...
                <p class="font-mono text-sm opacity-80">Seçili filtrelerle eşleşen yönetim kaydı bulunamadı.</p>
            </section>
        <?php else: ?>
                ...
                <?php foreach ($logs as $row): ?>
                    <?php
                        ...
                        $metaItems = [
                            ...
                            'Uç nokta' => $context['endpoint'] ?? '',
                            'Import' => $context['import_id'] ?? '',
                            'Görsel sırası' => isset($context['image_index']) ? (string)$context['image_index'] : '',
