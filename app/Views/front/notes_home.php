<?php 
    $page_title = "FEZADAN Notlar — Akademik Veri Havuzu";
    $page_description = $page_description ?? 'Fezadan Notlar portalında bilimsel PDF dosyalarını, akademik makaleleri ve araştırma notlarını arayın, okuyun ve indirin.';
    $current_lang = strtoupper((string)($current_lang ?? ''));
...
?>
    ...
    <main id="main-content" class="flex-1 relative z-10 max-w-7xl mx-auto px-6 py-12 w-full flex flex-col min-h-screen">
        ...
        <form action="/" method="GET" class="mb-12 flex flex-col lg:flex-row gap-4 bg-[var(--bg-secondary)] p-6 border-2 border-[var(--text-main)] shadow-[8px_8px_0px_var(--text-main)]">
            ...
            <input type="text" name="search" value="<?php echo htmlspecialchars($current_search ?? ''); ?>" placeholder="Notlarda ara..." aria-label="Notlarda ara" class="w-full lg:flex-1 px-4 py-3 bg-[var(--bg-paper)] border-2 border-[var(--text-main)] font-mono text-lg outline-none focus:shadow-[4px_4px_0px_var(--text-accent)] transition-all placeholder:opacity-50 text-[var(--text-main)]">
            
            <select name="cat" aria-label="Kategori" class="w-full lg:w-auto px-4 py-3 bg-[var(--bg-paper)] border-2 border-[var(--text-main)] font-mono text-sm font-bold uppercase cursor-pointer outline-none focus:shadow-[4px_4px_0px_var(--text-accent)] transition-all text-[var(--text-main)]">
            ...
            <select name="lang" aria-label="Dil" class="w-full lg:w-auto px-4 py-3 bg-[var(--bg-paper)] border-2 border-[var(--text-main)] font-mono text-sm font-bold uppercase cursor-pointer outline-none focus:shadow-[4px_4px_0px_var(--text-accent)] transition-all text-[var(--text-main)]">
                <option value="">TÜM DİLLER</option>
                <option value="tr" <?php echo $current_lang === 'TR' ? 'selected' : ''; ?>>TÜRKÇE</option>
                <option value="en" <?php echo $current_lang === 'EN' ? 'selected' : ''; ?>>İNGİLİZCE</option>
            </select>
        ...
        <div class="mt-16 flex justify-center gap-2 font-mono text-sm mb-8">
            <?php if($current_page > 1): ?>
                <a href="<?php echo htmlspecialchars($previous_page_url ?? '/', ENT_QUOTES, 'UTF-8'); ?>" class="px-4 py-2 border-2 border-[var(--text-main)] text-[var(--text-main)] hover:bg-[var(--text-main)] hover:text-[var(--bg-paper)] transition-colors font-bold bg-[var(--bg-paper)]">← ÖNCEKİ</a>
            <?php endif; ?>
            ...
            <?php if($current_page < $total_pages): ?>
                <a href="<?php echo htmlspecialchars($next_page_url ?? '/', ENT_QUOTES, 'UTF-8'); ?>" class="px-4 py-2 border-2 border-[var(--text-main)] text-[var(--text-main)] hover:bg-[var(--text-main)] hover:text-[var(--bg-paper)] transition-colors font-bold bg-[var(--bg-paper)]">SONRAKİ →</a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
